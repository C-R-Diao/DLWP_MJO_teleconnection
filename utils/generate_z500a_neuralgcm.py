def generate_z500a_neuralgcm(case_name = 'Hist-MJO-P3_NeuralGCM'):
    '''
    ...
    fpath_climo = '../data/ERA5/annual_cycle_climo/'
    ...
    # resort latitude to be from -90 to 90
    ds_climo = ds_climo.sortby("latitude")

    #===== load experiment outputs =====#
    for date in dates:
        print(date)
        fpath_exp = f'../data/exp_Hist-MJO-P3/{case_name}_{date}/'
        fin_name = f'output.{case_name}_{date}.day0-30.nc'  

        ds = xr.open_dataset(fpath_exp + fin_name)

        # read geopotential and convert to z500
        z500 = ds['geopotential'].sel(level=500).drop('level') / 9.8 # convert To geopotential height (meter) 
        ...
        time = [start_date + timedelta(days=i+1) for i in range(z500.shape[0])]
        time1 = xr.DataArray(time, dims='time', name='time')
        z500['time'] = time1       

        # regrid era5 to neuralgcm grid
        if date == dates[0]: # avoid repeat this in the loop 
            ds_climo_regridded = ds_climo.interp(latitude=z500.latitude, longitude=z500.longitude)

        # remove annual cycle by subtracting the daily climatology        
        time_climo = [t.replace(year=year_climo) for t in time]
        z500_climo = ds_climo_regridded['z500'].sel(time=time_climo).isel(level=0)
        ...
        fout_name = f'output.{case_name}_{date}.z500_anomaly.day0-30.nc'
...
#====================================================#
def create_z500a_casemean_neuralgcm(case_name = 'Hist-MJO-P3_NeuralGCM'):
    '''
    ...
    fout_name = f'output.{case_name}.z500a.case-mean.day0-30.nc'
